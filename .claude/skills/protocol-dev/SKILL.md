...
- 本项目运行环境固定为 **Python 3.10.x**；新增或修改 Python 代码时，必须确认语法与依赖兼容 Python 3.10。
- 项目包含发布用 `.pyc` 字节码文件；除非任务明确要求构建发布包或重新混淆，禁止随意改动、重生成或提交 `.pyc` 文件。
- 启动服务前必须先 `unset all_proxy`，避免 Playwright / HTTPX 走代理导致 `socksio` 等运行错误。
- 修改服务启动、路由、Dreamina/Seedance 执行链路后，优先使用 `.venv` 中的 Python 3.10 执行最小验证；涉及服务运行时，启动后必须检查日志中是否出现 `[region-patch] Patched 4 Dreamina URLs`。
- 涉及 Roxy Browser / Playwright CDP 的改动，必须考虑 GUI 桌面环境与 dashboard 标签页导致 CDP 挂起的问题；打开窗口后应检查并关闭 dashboard 标签页。
